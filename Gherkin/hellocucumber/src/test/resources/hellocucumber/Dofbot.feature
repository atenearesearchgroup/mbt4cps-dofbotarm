Feature: Series of operations for the arm
  ...
  @Movement
	...
	Scenario: Rotate a single servo:
		...
		When  We call operation rotateServoOperation(1,45,2000)
		Then  The result is 2000
		And   Not_Later_Than 2000 Query isAtSingleOperation(1, 90, AnglePrecision) returns True
		
		When  We call operation rotateServoOperation(2,90,1000)
		Then  The result is 1000
		And   Not_Later_Than 1000 Query isAtSingleOperation(1, 90, AnglePrecision) returns True
...
Class: Machine
	StateMachine: RotateASingleServo
		...
		State: When2
			Activity: Arm.BaseServo.ServosOperations.rotateServoOperation(1, 45, 2000)
		
		Transition: (When2->Error)
			Guard: LaterThan(oRunTime, 2000)		
			
		Transition: (When2->When3)
			Guard: Arm.BaseServo.ServosOperations.isAtSingleOperation(1, 45, 2000)	
